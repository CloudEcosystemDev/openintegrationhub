...
const {
    userIsOwnerOfAuthClient,
    userIsInEntityTenant,
...
class AuthClientRouter {
    ...
    setup() {
        ...
        this.router.get(
            '/:id',
            (req, res, next) => userIsInEntityTenant(AuthClientDAO, req, res, next),
            async (req, res) => {
        ...
        this.router.patch(
            ...
            async (req, res, next) => {
                ...
                try {
                    if (
                        data.owners
            && !data.owners.find((owner) => owner.persistentEntry)
                    ) {
                        const authClient = await AuthClientDAO.findOne({
                            _id: req.params.id,
                        });
                        data.owners.push(
                            authClient.owners.find((owner) => owner.persistentEntry),
                        );
                    }
                    res.send({
                        data: maskAuthClient({
        ...
        this.router.delete(
            '/',
            can([restricted.authClientDeleteAny]),
            async (req, res, next) => {
        ...
        this.router.get(
            '/:id/secrets',
            /* userIsOwnerOfAuthClient, */ async (req, res, next) => {
                try {
                    const secrets = await findByAuthClient(req.user.sub, req.params.id);
                    res.send({
                        data: secrets,
                    });
                } catch (err) {
        ...
        this.router.post(
            '/:id/start-flow',
            getKeyParameter,
            /* auth.userIsOwnerOfAuthClient, */ jsonParser,
            async (req, res, next) => {
